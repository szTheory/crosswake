...
#   DRY_RUN=1 script/register-native-gate.sh    # print desired JSON, exit 0 without writing
#
# Ordering (run by maintainer out-of-band — harness-blocked):
#   1. Merge the workflow PR (native-behavioral-proof-gate.yml with aggregator topology).
...
#
# This script + documents the PATCH — it does NOT auto-toggle branch protection (D-08).
...
#     so enforce_admins and review requirements are left untouched (T-123-10 mitigation).
#   - Appends {context: merge-blocking-native-behavioral-proof, app_id: 15368} to required checks.
...
#
# merge-blocking-native-behavioral-proof is the SOLE new required check (NTEST-04 / D-07).
# The iOS advisory lane (ios-package-unit) is NOT registered as required;
# native-toolchain checks (Xcode) remain advisory per the hermetic-vs-advisory split.
#
...
# --- Green-first preflight (T-123-11 mitigation) ---
# Refuse to register until merge-blocking-native-behavioral-proof has gone green on BRANCH
...
# Checks the AGGREGATOR job's conclusion (merge-blocking-native-behavioral-proof), not a sibling (D-08).
echo ""
...
if ! gh api "repos/${REPO}/commits/${BRANCH}/check-runs" \
  ...
  echo ""
  echo "To resolve:"
  echo "  1. Make sure the workflow PR is merged (native-behavioral-proof-gate.yml)."
  echo "  2. Wait for merge-blocking-native-behavioral-proof to go GREEN on main."
...
echo ""
echo "Done. merge-blocking-native-behavioral-proof is now a required check on ${BRANCH}."
