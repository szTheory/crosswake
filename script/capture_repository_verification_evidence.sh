...
capture() {
  ...
  if [[ -f "$checkout/mix.exs" ]]; then
    ...
    (cd "$checkout/examples/phoenix_host" && MIX_ENV=dev mix deps.get && MIX_ENV=test mix deps.get && npm ci && npx playwright install chromium) >"$run_root/bootstrap-host.log" 2>&1 || return 1
  ...
  GIT_OPTIONAL_LOCKS=0 git -C "$checkout" status --porcelain=v1 -z --untracked-files=all >"$baseline_snapshot"
  ...
  GIT_OPTIONAL_LOCKS=0 git -C "$checkout" status --porcelain=v1 -z --untracked-files=all >"$final_snapshot" || status=1
