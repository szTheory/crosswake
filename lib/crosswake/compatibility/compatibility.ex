defmodule Crosswake.Compatibility do
  ...
  def finding_to_denial(%Finding{} = finding, opts \\ []) do
    ...
    {reason, recovery, details} =
      case finding.axis do
        ...
        :pack_version ->
          {:pack_incompatible, recovery_for(:pack_incompatible, opts), pack_details(finding, opts)}
  ...
  defp compatible_pack?(available_version, required_version), do: compatible_version?(available_version, required_version)

  defp pack_message(pack_id, nil, nil), do: "route requires pack #{pack_id}, but it is not installed in the shell"
  ...
  defp pack_message(pack_id, required_version, nil),
    do: "route requires pack #{pack_id} at #{required_version}, but it is not installed in the shell"

  defp pack_message(pack_id, required_version, available_version),
    do: "route requires pack #{pack_id} at #{required_version}, but the shell exposes #{available_version}"
  ...
  defp recovery_for(:pack_incompatible, opts) do
    ...
    case Keyword.get(opts, :activation_source) do
      :deep_link ->
        case Keyword.get(opts, :fallback_route_id) do
          nil -> Map.put(base, :mode, :safe_retry)
          fallback_route_id ->
            %{mode: :safe_fallback, fallback_route_id: fallback_route_id, actions: [:retry, :open_safe_fallback, :update_app]}
        end
