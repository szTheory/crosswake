defmodule Crosswake.Companions.PlayBilling.Evidence do
  @moduledoc false

  alias Crosswake.Commerce.Contracts
  alias Crosswake.Commerce.ProviderEvidence

  @enforce_keys [:purchase_token, :event_kind, :environment, :source, :captured_at]
  defstruct [
    :purchase_token,
    :rtdn_message_id,
    :order_id,
  ...
  @type t :: %__MODULE__{
          ...
          rtdn_message_id: String.t() | nil,
          order_id: String.t() | nil,
          payload_digest: String.t() | nil,
          event_kind: String.t(),
          environment: :sandbox | :production | :license_test,
    ...
    with {:ok, evidence_ref} <- evidence_ref(evidence) do
      {:ok,
       %Contracts.ReconciliationEvidence{
         ...
         provider_reference: evidence.purchase_token,
         event_kind: evidence.event_kind,
         evidence_ref: evidence_ref,
         captured_at: evidence.captured_at,
         integrity_digest: evidence.payload_digest,
  ...
  defp build(attrs, event_kind, source, environment) do
    try do
      {:ok,
       struct!(__MODULE__, %{
         purchase_token: Map.get(attrs, :purchase_token),
         rtdn_message_id: Map.get(attrs, :rtdn_message_id),
         order_id: Map.get(attrs, :order_id),
         payload_digest: Map.get(attrs, :payload_digest),
         event_kind: event_kind,
         environment: environment,
         source: source,
         captured_at: Map.get(attrs, :captured_at),
         idempotency_ref: Map.get(attrs, :idempotency_ref),
         metadata: Map.merge(%{play_environment: environment}, Map.get(attrs, :metadata, %{}))
       })}
    rescue
  ...
  defp require_evidence_identity(attrs) do
    if present?(Map.get(attrs, :rtdn_message_id)) or present?(Map.get(attrs, :order_id)) or
         present?(Map.get(attrs, :payload_digest)) do
      :ok
    else
