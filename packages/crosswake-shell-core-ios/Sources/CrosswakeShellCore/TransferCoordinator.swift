...
public final class TransferCoordinator: ObservableObject {
    public enum TransferCommand: String, CaseIterable {
    ...
    public enum TransferState: String, CaseIterable {
        case queued = "queued"
    ...
    public struct TransferRecord: Equatable {
        let routeID: String
    ...
    public struct StagedDocument: Equatable {
        let routeID: String
        let transferID: String
        let handle: String
        let stagedPath: String
    ...
    @Published public private(set) var transfers: [String: TransferRecord] = [:]
    @Published public private(set) var stagedDocuments: [String: StagedDocument] = [:]

    public init(routeID: String, declaredTransfers: [ShellManifest.TransferSeam]) {
        self.routeID = routeID
        self.declaredTransfers = declaredTransfers
    }

    public func execute(command: String, payload: [String: String], correlationID: String) -> [String: String]? {
    ...
    public func stageCapturedMedia(
        transferID: String,
        localPath: String,
        mediaType: String,
        bytes: Int
    ...
    public func declaredPickerTransfer(id: String) -> ShellManifest.TransferSeam? {
        declaredTransfers.first(where: { seam in
            seam.id == id && seam.direction == "inbound" && seam.source == "native_picker"
        })
    }

    public func markPickerCanceled(transferID: String, correlationID: String) {
    ...
    public func stagePickedDocument(
        transferID: String,
        sourceURL: URL,
