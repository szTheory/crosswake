...
enum PackState: String, Codable, CaseIterable {
...
struct PackInventoryRecord: Codable {
    let packID: String
...
struct RequiredPackStatus: Equatable, Identifiable {
    let id: String
    let packID: String
    let requiredVersion: String
    var state: PackState
...
final class PackStore: ObservableObject {
    @Published private(set) var statuses: [String: RequiredPackStatus]

    private let requiredVersions: [String: String]
    private let now: () -> Date

    init(requiredVersions: [String: String], inventory: [PackInventoryRecord], now: @escaping () -> Date = Date.init) {
    ...
    static func bundled(bundle: Bundle = .main) throws -> PackStore {
    ...
    func blockingStatus(for packReferences: [String]) -> RequiredPackStatus? {
        packReferences
    ...
    func installRequiredPack(_ status: RequiredPackStatus) async {
    ...
    func retry(_ status: RequiredPackStatus) async {
        await installRequiredPack(status)
    }

    func invalidatePack(_ status: RequiredPackStatus) async {
