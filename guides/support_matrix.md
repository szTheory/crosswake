...
| Target | Version | Baseline | Proof Status | Proof Hook | Boundaries | Notes |
|--------|---------|----------|--------------|------------|------------|-------|
| ios_shell | Hex-matched | supported | verification required | clean-room-proof-ios; script/verify_generated_ios_shell.sh | [View Boundaries](native_shell.md#boundary-warnings--rough-edges) | Default non-local scaffolds resolve `https://github.com/szTheory/crosswake-shell-core-ios.git` via SwiftPM at the Crosswake package version; release-time clean-room proof confirms external resolution and `swift build`. The Phase 145 SwiftPM mirror tag/backfill path is release-registry evidence only; it is not device, simulator, or emulator proof. advisory — not wired as a required CI lane; macOS/Xcode toolchain not guaranteed in CI. |
...
- Standalone native shell core packages are consumed by generated host-owned wrappers through SwiftPM and Maven Central; SwiftPM mirror verification/backfill proves release-registry state for the tag, not device/emulator behavior, and broader native runtime claims remain deferred until separately proven.
