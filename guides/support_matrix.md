...
| deep_link | bounded_bridge | core | merge-blocking | none | bundled or cached manifest; shell activation support | route_unavailable | show route unavailable surface and keep route ownership explicit | [Guide](native_shell.md#manifest-first-activation) |
...
| entitlement_snapshot | backend_seam | example/docs-only | advisory | companion-required | backend entitlement authority; reconciliation hook | unavailable_capability | treat device-side state as evidence instead of final entitlement truth | [Guide](capabilities.md#backend-seams-and-deferred-surfaces) |
...
| notification_token | bounded_bridge | companion | advisory | companion-required | push entitlement setup; backend reconciliation | unavailable_capability | treat token delivery as evidence, not notification truth | [Guide](capabilities.md#bounded-bridge) |
| paywall_entry | backend_seam | example/docs-only | advisory | companion-required | backend entitlement contract; storefront guidance | unavailable_capability | fall back to Phoenix-owned paywall guidance without device authority | [Guide](capabilities.md#backend-seams-and-deferred-surfaces) |
| permissions.status | bounded_bridge | example/docs-only | advisory | none | point-of-need permission snapshot; declared route capability | unavailable_capability | route continues without native permission snapshot authority | [Guide](capabilities.md#bounded-bridge) |
| purchase_intent | backend_seam | example/docs-only | advisory | companion-required | backend reconciliation; provider-specific adapter | unavailable_capability | treat purchase events as reconciliation inputs, not entitlement truth | [Guide](capabilities.md#backend-seams-and-deferred-surfaces) |
| reconciliation_evidence | backend_seam | example/docs-only | advisory | companion-required | backend reconciliation; device callback or webhook | unavailable_capability | treat evidence as asynchronous payload without blocking route entry | [Guide](capabilities.md#backend-seams-and-deferred-surfaces) |
| restore_intent | backend_seam | example/docs-only | advisory | companion-required | backend reconciliation; storefront-aware adapter | unavailable_capability | keep restore flow backend-owned until adapter truth is explicit | [Guide](capabilities.md#backend-seams-and-deferred-surfaces) |
