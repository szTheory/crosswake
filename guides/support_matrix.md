...
| auth.sigra.session_authority | merge-blocking | merge-blocking | session_authority | auth posture route fixtures; session authority evaluator proof; step_up_required denial-code proof | 1 | current release branch | zero contract failures | ios; android | guides/support_matrix.md; guides/companions.md | native or companion rebuild required | companion_native | diag.auth.sigra_session_authority | Demote if route predicates, auth_posture, step_up_required denial codes, or session-authority docs drift from support truth. |
