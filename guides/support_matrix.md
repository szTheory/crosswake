...
| auth.sigra.contract_only | merge-blocking | merge-blocking | contract_only | auth predicate route fixtures; step_up_required denial proof | 1 | current release branch | zero contract failures | ios; android | guides/support_matrix.md; guides/companions.md | native or companion rebuild required | companion_native | diag.auth.sigra_contract_only | Demote if route predicates, step_up_required denial, or contract-only docs drift from support truth. |
| purchase_intent.provider.storekit | advisory | merge-blocking | provider_adapter | StoreKit adapter implementation; backend reconciliation proof; storefront advisory lane | 3 | current adapter release | zero entitlement-authority failures | ios | guides/support_matrix.md; guides/commerce.md | native or companion rebuild required | provider_adapter | diag.provider.adapters_not_shipped | Remain advisory until StoreKit adapter, provider setup, docs parity, and backend reconciliation proof all pass. |
| restore_intent.provider.storekit | advisory | merge-blocking | provider_adapter | StoreKit restore adapter implementation; backend reconciliation proof; storefront advisory lane | 3 | current adapter release | zero entitlement-authority failures | ios | guides/support_matrix.md; guides/commerce.md | native or companion rebuild required | provider_adapter | diag.provider.adapters_not_shipped | Remain advisory until StoreKit restore evidence stays backend-owned and proof/docs parity pass. |
| purchase_intent.provider.play_billing | advisory | merge-blocking | provider_adapter | Play Billing adapter implementation; backend reconciliation proof; storefront advisory lane | 3 | current adapter release | zero entitlement-authority failures | android | guides/support_matrix.md; guides/commerce.md | native or companion rebuild required | provider_adapter | diag.provider.adapters_not_shipped | Remain advisory until Play Billing adapter, provider setup, docs parity, and backend reconciliation proof all pass. |
| restore_intent.provider.play_billing | advisory | merge-blocking | provider_adapter | Play Billing restore adapter implementation; backend reconciliation proof; storefront advisory lane | 3 | current adapter release | zero entitlement-authority failures | android | guides/support_matrix.md; guides/commerce.md | native or companion rebuild required | provider_adapter | diag.provider.adapters_not_shipped | Remain advisory until Play Billing restore evidence stays backend-owned and proof/docs parity pass. |
