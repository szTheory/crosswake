...
| auth.sigra.session_authority | merge-blocking | merge-blocking | session_authority_handoff_step_up_contract | auth posture route fixtures; session authority evaluator proof; step_up_required denial-code proof; single-use handoff ticket lifecycle proof; server-record redemption and audit proof; server-owned step-up intent lifecycle proof; shared Plug/LiveView ceremony proof | 1 | current release branch | zero contract failures | ios; android | guides/support_matrix.md; guides/companions.md | native or companion rebuild required | companion_native | diag.auth.sigra_session_authority | Demote if route predicates, auth_posture, step_up_required/auth.handoff/auth.step_up_intent denial codes, handoff or step-up server-record proof, or Sigra docs drift from support truth. |
