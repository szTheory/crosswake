defmodule Crosswake.Proof.Phase153_1GateIntegrityTest do
  ...
  defp run_detector(tmp, args \\ ["--emitters"]) do
    System.cmd("python3", [@discover_relative | args],
      cd: tmp,
  ...
  defp emitters do
    {out, 0} =
      System.cmd("python3", [@discover, "--emitters"], cd: @repo_root)
  ...
  test "negative control: the uniqueness assertion fails on two jobs sharing a name", %{
    ...
    {out, status} =
      System.cmd("bash", [@checker_relative], cd: tmp, stderr_to_stdout: true)
  ...
  test "the gate scripts are bash 3.2 compatible (macOS ships 3.2.57)" do
    ...
    for script <- [
          @checker,
          Path.join(@repo_root, "script/register_required_checks.sh"),
          Path.join(@repo_root, "script/verify_generated_ios_shell.sh")
        ] do
  ...
  test "local-only performs complete local validation without gh authority", %{tmp_dir: tmp} do
    ...
    {out, status} =
      System.cmd("bash", [@checker_relative, "--local-only"],
        cd: tmp,
