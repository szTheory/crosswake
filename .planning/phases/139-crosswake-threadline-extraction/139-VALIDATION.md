...
| 139-01-02 | 01 | 1 | THREAD-01 | T-139-02, T-139-15 | Core decoupled atomically: `@audit_ledger_support_truth` frozen as static literals (support_matrix.ex); static `Crosswake.Threadline.Telemetry.forbidden_metadata_keys()` call CUT from `telemetry.ex` `attach_default_logger/1`; core `@baseline_forbidden_keys` gains ONLY the curated universal-floor delta (auth-token/session/identity), NOT threadline's OAuth/passkey minutiae (D-5); core compiles clean, no `Crosswake.Threadline.*` compile dep remains | compile + structural | `mix compile --warnings-as-errors` (repo root); `grep -n "Crosswake.Threadline.Telemetry" lib/crosswake/telemetry.ex lib/crosswake/support_matrix/support_matrix.ex` empty | ❌ W0 | ⬜ pending |
...
| 139-02-01 | 02 | 2 | THREAD-01, THREAD-02 | T-139-07, T-139-15 | Moved threadline unit tests (id/telemetry/plug/ledger + task tests) pass in package lane; anti-drift test asserts `core_baseline ⊆ union(registered-companion forbidden_metadata_keys)` (D-5; inverse NOT asserted); any core-side test asserting the baseline key count updated to the new curated-floor count | unit | `cd packages/crosswake_threadline && mix test`; `mix test --exclude requires_example_host` (core, green) | ❌ W0 | ⬜ pending |
...
| 139-02-03 | 02 | 2 | THREAD-02 | T-139-08 | Vacuity-safe clean-room ExUnit proof: `Telemetry.event_names/0` == 3 canary, `Plug.Threadline.init([])` OK, `Audit.Ledger.actor_ref/2` returns 64-char hex; `refute :crosswake_sigra in deps` + `refute :crosswake_chimeway in deps` (proves zero-sibling-dep non-vacuously; `async: true`) | integration | `cd packages/crosswake_threadline && mix test test/crosswake/proof/phase139_threadline_cleanroom_test.exs` | ❌ W0 | ⬜ pending |
| 139-02-04 | 02 | 2 | THREAD-01 | T-139-16 | DX (own commits): `mix crosswake.threadline` NO_COLOR/ASCII tree fallback (`+--`/`\--`/`\|   `) + empty-result guard (`No events found for thread_id=…`) + brand-voice posture microcopy (ephemeral names config keys; durable = `Posture: Durable — querying audit ledger`); no new PII field in output | unit | `cd packages/crosswake_threadline && mix test test/mix/tasks/crosswake.threadline_test.exs` | ❌ W0 | ⬜ pending |
| 139-02-05 | 02 | 2 | THREAD-01 | T-139-17 | DX (own commits): brand-voice `gen.audit` "Next steps" rewrite (verb-first, real `record_in_multi` fields, `mix ecto.create` note, closing `mix crosswake.threadline` CTA, `reused`→`skipping`) + HMAC-secret `ArgumentError` names both the `:secret` option and `:audit_hmac_secret` env key (fail-loud) | unit | `cd packages/crosswake_threadline && mix test test/mix/tasks/crosswake.gen.audit_test.exs test/crosswake/audit/ledger_test.exs` | ❌ W0 | ⬜ pending |
...
- [ ] Core decoupling: freeze `@audit_ledger_support_truth` static literals; CUT the static `Crosswake.Threadline.Telemetry.forbidden_metadata_keys()` call in `telemetry.ex`; expand core `@baseline_forbidden_keys` by ONLY the curated universal-floor delta (final count determined by per-key categorization per D-5 — NOT a wholesale absorb of threadline's ~20 keys)
