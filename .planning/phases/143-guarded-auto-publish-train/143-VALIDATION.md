...
| 143-02-01 | 02 | 2 | AUTO-03 | T-143-07, T-143-08, T-143-11 | Manual recovery exposes package/ref/release_version inputs, accepts only full 40-character lowercase SHA or explicit `refs/tags/` release refs, rejects `release/v0.2.0`, `feature/v0.2.0`, `refs/heads/release/v0.2.0`, bare `v0.2.0`, `refs/heads/*`, `heads/*`, `main`, and `master` before checkout, and stays least-privilege. | workflow text assertions | `bash -lc 'test "$(grep -c "package:" .github/workflows/hex-publish.yml)" -ge 1 && test "$(grep -c "ref:" .github/workflows/hex-publish.yml)" -ge 1 && test "$(grep -c "release_version:" .github/workflows/hex-publish.yml)" -ge 1 && test "$(grep -c "inputs.tag" .github/workflows/hex-publish.yml || true)" -eq 0 && test "$(grep -c "contents: write" .github/workflows/hex-publish.yml || true)" -eq 0 && for sample in "release/v0.2.0" "feature/v0.2.0" "refs/heads/release/v0.2.0" "refs/heads/" "heads/" "main" "master"; do grep -Fq "$sample" .github/workflows/hex-publish.yml; done && grep -Eq "(^|[^A-Za-z0-9_/.-])v0\\.2\\.0([^A-Za-z0-9_/.-]|$)" .github/workflows/hex-publish.yml && grep -Fq "refs/tags/" .github/workflows/hex-publish.yml && grep -Fq "^[0-9a-f]{40}$" .github/workflows/hex-publish.yml'` | Existing workflow | planned |
...
| 143-03-01 | 03 | 3 | AUTO-01, AUTO-02, AUTO-03 | T-143-13, T-143-14, T-143-15, T-143-16, T-143-18, T-143-19 | Scanner has stable Phase 143 IDs for guarded auto publish, full-SHA-or-`refs/tags/` recovery, rejection of `release/v0.2.0`, `feature/v0.2.0`, `refs/heads/release/v0.2.0`, and bare `v0.2.0`, package map completeness, no routine overwrite, proof continuation, lockstep core/native, independent companions, and honest floors. | semantic scanner | `elixir script/check_release_workflow_integrity.exs` | Existing scanner | planned |
| 143-03-02 | 03 | 3 | AUTO-01, AUTO-02, AUTO-03 | T-143-13, T-143-14, T-143-15, T-143-16, T-143-18, T-143-19 | ExUnit positive and negative fixtures prove the scanner catches direct publish, root-only recovery, mutable refs including `release/v0.2.0`, `feature/v0.2.0`, `refs/heads/release/v0.2.0`, and bare `v0.2.0`, missing package map, routine overwrite syntax, aggregate behavioral gates, and floor flattening. | ExUnit proof | `mix test test/crosswake/proof/phase142_release_integrity_test.exs` | Existing test file | planned |
...
- [x] `recovery.hex.exact_ref_only` is planned to fail on `release/v0.2.0`, `feature/v0.2.0`, `refs/heads/release/v0.2.0`, and bare `v0.2.0`, while accepting/referring only to explicit `refs/tags/` release refs and full 40-character lowercase SHA refs.
