...
- **D-01a — Wiring.** **RESOLVED 2026-06-14 (supersedes the "new file" wording below):** the clean-room proof is implemented as **two appended JOBS inside `release-please.yml`** — `clean-room-proof-ios` (macOS) and `clean-room-proof-android` (ubuntu) — NOT a separate `clean-room-proof.yml` file. Rationale: GitHub Actions `needs:` and `needs.<job>.outputs.*` only resolve between jobs in the **same** workflow file; a separate file literally cannot `needs: [release-please, publish-hex, ...]` nor read `needs.release-please.outputs.version`. The house `phaseNN-proof.yml` files are PR-merge gates that depend on nothing upstream, so they are not a precedent for an output-dependent release-time job. The decision *intent* is unchanged: `needs: [release-please, publish-hex, publish-ios-core, publish-android-core]`, gated `if: needs.release-please.outputs.releases_created`, pinned to the just-cut version. Scaffolds a host project in `$RUNNER_TEMP` **outside** the monorepo (no `--local`, no `packages/` access), runs `mix crosswake.gen.shell ios` and `... android`, then `swift build` (macOS runner) + `gradle build` (ubuntu runner) to confirm the published deps resolve and compile.
...
- Clean-room lane: two appended jobs (`clean-room-proof-ios`, `clean-room-proof-android`) **inside** `.github/workflows/release-please.yml` (per D-01a resolution — NOT a separate file; they `needs:` the existing publish jobs in the same workflow).
