...
## Milestone: v16.0 — Companion Extraction & Package-Family Discipline

**Shipped:** 2026-06-30
**Phases:** 7 (129-135) | **Plans:** 24

### What Was Built
- `rulestead` then `rindle` extracted from in-tree to standalone `packages/crosswake_*` Hex projects (own `mix.exs`/`@version`, source + tests moved out of core) with module names preserved (`Crosswake.Companions.Rulestead`/`.Rindle`) so the sole adopter touch-point is unchanged — extraction is non-breaking.
- A frozen public companion-contract surface (5 named modules, semver-governed) plus stdlib-only AST guards (`check_source/1`, `check_ensure_loaded_placement/1` via `Macro.prewalk`) that keep core compile-decoupled from companions and `Code.ensure_loaded?` runtime-only — EXTRACT-03/04 merge-blocking and non-vacuous.
- Fail-closed discipline: RouteGate `:dependency_missing` denial (an absent companion denies, never crashes), doctor `:error` for enabled-but-missing, independent release-please components (not core lockstep), and a drift-tested cross-package compatibility matrix.
- Publish pipeline + clean-room install lane gated by `hex.publish --dry-run`; `Crosswake.Telemetry` as a semver-governed public event contract + opt-in PII-safe logger (core never auto-attaches); generated-shell `@template_version` stamping + `shell.status` + non-destructive `gen.shell --diff` + upgrade guide; and PROOF-03 0-human release-as automation (staleness guard, auto-cleanup PR, failure alerts, parametric required-check registration).

### What Worked
- **Dress-rehearsal-before-irreversible-publish (Phase 130)** — standing up `crosswake_rulestead` as a `path:` dep first, with no Hex publish, surfaced the seam/footgun issues before any unrecoverable `hex.publish`. The irreversible step landed on a settled mechanism.
- **Module-name preservation made extraction non-breaking** — moving source out of core while keeping `Crosswake.Companions.Rulestead` meant `config :crosswake, :companions, [...]` never changed; the package boundary is free to adopters.
- **The milestone audit caught the real CI blocker** — what the audit first logged as a "minor flaky test" (the `File.cd!` race under `async:true`) turned out to redden *every* broad-`mix test` merge-blocking lane on PR #40's first real run. The audit's tech-debt follow-up, not the green local suite, is what found it.
- **Shift-left automation caught a genuine setup bug** — the `android-generated-shell-unit` lane failed its first real run with `Can't continue due to errors on dependencies` (missing `mix deps.get`), exactly the class of post-publish setup bug the LIFE-01 native UAT promotion was designed to surface automatically.

### What Was Inefficient
- **"Minor flaky test" was the chief release risk, mislabeled** — the `File.cd!`/`async:true` race surfaced as a *misattributed parallel-compile error* on a broad lane, so it read as noise until it reddened the merge. A flaky test on a merge-blocking lane is a release blocker, not a minor item.
- **`decision-coverage-plan` gate false-negatived twice** (Phases 130 & 131) — the blocking gate dropped long-bold/embedded-colon `**D-NN: …**` CONTEXT bullets as "unparseable" and matched 0/N, forcing a manual cross-check against the independent plan-checker's Dimension 7 each time. A known parser brittleness that still cost override work.
- **CLI placeholder-bullet tax, again** — `milestone.complete` emitted 19 raw per-task SUMMARY fragments (bare filenames, `Task 1 — …(commit)`, a stray `[Rule 1 - Bug]` line) into MILESTONES.md, requiring a full manual rewrite — the same chronic tax seen v3.4→v14.0.
- **Local-main-ahead-of-origin ran the whole milestone** — v16.0 work accumulated ~64 commits on local `main` ahead of `origin`, syncing in one boundary PR (#40); convenient for WIP but it deferred all real-CI signal to the very end, which is precisely when the `File.cd!` race detonated.

### Patterns Established
- **Independent per-companion versioning over lockstep for a package family** — companions advance on `@behaviour` seams + optional deps (the Ash/Oban/Swoosh/Nerves shape), with a drift-tested compatibility matrix replacing forced version coupling. Lockstep is for the three generated-coordinate registries, not the family.
- **Stdlib-only AST guards for structural invariants** — `Macro.prewalk`/prune-then-walk over source beats grep for "core never references an extracted companion" and "no module-eval `ensure_loaded?`"; deterministic, browser-free, merge-blocking.
- **Telemetry as a semver-governed public API, opt-in only** — events are a documented stable surface (the dashboard prerequisite), the logger is opt-in, and core never auto-attaches — no logging policy or PII risk imposed on adopters.
- **0-human release ops on the success path** — staleness guard + auto-cleanup PR + `if: failure()` alert + parametric required-check registration leave only the irreversible publish (merging the Release PR) as the human gate; recipe Step 12f points future companions at the automation so the pattern propagates for free.

### Key Lessons
- **A flaky test on a merge-blocking lane is a release blocker, full stop** — especially when it surfaces as a misattributed error on an unrelated lane. `async:false` for any test that does `File.cd!`; match the isolation posture of its siblings.
- **Defer-all-CI-to-the-boundary trades convenience for a late, concentrated risk** — accumulating a milestone on local `main` means the first real multi-lane CI signal arrives at the worst possible moment; consider periodic origin-sync to spread the signal.
- **Shift-left native UAT pays for itself on the first real run** — the Android `deps.get` bug would have been a silent post-publish adopter footgun; promoting the lane to a real run caught it pre-ship.
...
| Independent versioning + drift-tested matrix beats lockstep for a package family | v16.0 versioned `crosswake_rulestead`/`crosswake_rindle` independently (not the core `linked-versions` group) with a drift-tested compatibility matrix, while keeping the three generated-coordinate registries lockstep | Reserve lockstep for coordinates a single generated artifact ties together; let `@behaviour`-seam companions advance independently with a machine-checked compatibility matrix |
| The `decision-coverage-plan` gate false-negatives on long-bold/embedded-colon bullets | v16.0 Phases 130 & 131 both hit `covered 0/N` despite full citation, needing manual cross-check against the plan-checker's Dimension 7 (also seen as a documented project brittleness) | Cross-check the blocking decision-coverage gate against the independent plan-checker before treating `0/N` as a real gap; the parser drops `**D-NN: …**` CONTEXT bullets |
...
| CLI placeholder-bullet tax is chronic | v3.4, v3.9, v10.0, v11.0, v12.0, v14.0, and v16.0 all emitted placeholder or per-task fragment bullets into MILESTONES.md at close | Either fix SUMMARY one-liner authoring at phase close or fix the extraction CLI; milestone summaries must be reviewed before commit |
