...
| 167-07-02 | 167-07 | 7 | DOC-03 | T-167-20, T-167-22 | #105 is exact-head green/merged; receipt stays on phase branch and local main advances only from clean no-pending state | Swift + live receipt | `swift test --package-path packages/crosswake-shell-core-ios && python3 script/check_phase167_pr_dispositions.py --verify-resolution .planning/workstreams/quality-ratchet-release/phases/167-documentation-and-pull-request-reconciliation/evidence/pr-105-resolution.json --live` | ❌ receipt created by Plan 07 | ⬜ pending |
...
| T-167-28 | Protected local main receives executor commits or reconciliation loses history/runtime state | Require unprotected `agent-phase167-fixforward`, receipt commit before clean no-pending local-main ref update, all later commits on phase branch, recorded ancestry, empty index, and exact runtime hashes without config weakening/reset/clean/stash/switch. |
