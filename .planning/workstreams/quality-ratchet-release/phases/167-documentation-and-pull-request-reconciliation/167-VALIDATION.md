...
| 167-06-3D | 167-06 | 6 | DOC-01, DOC-03 | T-167-29, T-167-31 | Four primary proofs remain passing while a RED job-equivalent cleanup tracer selects a fully provisioned owner or explicit common prerequisite closure inside the exact generated five-path universe | Node RED/GREEN + closed ledger | `node --test test/js/repository_verification.test.mjs --test-name-pattern="generated.contract|repository cleanliness" && python3 script/check_phase167_default_reconciliation.py --self-test && python3 script/check_phase167_default_reconciliation.py --verify-post-412-ledger .planning/workstreams/quality-ratchet-release/phases/167-documentation-and-pull-request-reconciliation/evidence/fix-forward-failure-ledger.json --owner generated_contract_cleanup` | ❌ post-412 RED/GREEN pending | ⬜ pending |
| 167-06-3E | 167-06 | 6 | DOC-01, DOC-03 | T-167-30, T-167-31 | Local RED precedes a separate locally verified instrumentation commit; one instrumented diagnostic head preserves browser failure authority, emits only closed category/owner facts, and suppresses private Playwright artifact publication before later repair GREEN | Node RED/instrumentation/GREEN + workflow lint + one live diagnostic receipt | `actionlint .github/workflows/crosswake-ci.yml && node --test test/js/repository_verification.test.mjs --test-name-pattern="browser stage|browser proof|hosted diagnostic|failure artifact|redaction" && python3 script/check_phase167_default_reconciliation.py --verify-post-412-ledger .planning/workstreams/quality-ratchet-release/phases/167-documentation-and-pull-request-reconciliation/evidence/fix-forward-failure-ledger.json --owner browser --live` | ❌ local RED, safe instrumentation, one hosted diagnostic, and repair GREEN pending | ⬜ pending |
...
| T-167-30 | Hosted browser diagnosis leaks private logs/artifacts, cannot identify an owner, or promotes an empty-version clue into causality | Fail locally first, then separately commit and locally verify a closed structured mapper plus negative redaction controls; suppress Playwright failure-artifact publication only on the exact diagnostic path without skipping/changing proof failure, push once, and require otherwise-identical clue RED/GREEN before repair. |
| T-167-31 | Repeated recovery attempts widen scope or mutate PR state before proof | Permit one diagnostic and one final PR #149 head update; exhaustion, a new owner, or any red/missing final check checkpoints with all three recovery PRs open/unmerged. |
