...
| 167-06-3A | 167-06 | 6 | DOC-01, DOC-03 | T-167-16, T-167-17 | Both failed runs map every leaf to one shared owner; recurring Phase 166 queue/schema proof passes a shallow checkout while pinned source proof retains historical range/tree validation | Python synthetic + shallow RED/GREEN + clean-checkout ledger | `python3 script/check_phase167_default_reconciliation.py --self-test && python3 script/check_phase167_default_reconciliation.py --verify-failure-ledger .planning/workstreams/quality-ratchet-release/phases/167-documentation-and-pull-request-reconciliation/evidence/fix-forward-failure-ledger.json` | ❌ revised validator/ledger created by Recovery 3A | ⬜ pending |
| 167-06-3B | 167-06 | 6 | DOC-01, DOC-03 | T-167-17, T-167-18 | Only reproduced defects inside the closed authorized repair universe are edited; exact full source passes shallow recurring and pinned full-history proof | Python fixed-argv exact-commit proof | `python3 script/check_phase167_default_reconciliation.py --verify-candidate .planning/workstreams/quality-ratchet-release/phases/167-documentation-and-pull-request-reconciliation/evidence/default-branch-dependency-closure.json --candidate HEAD --local-clean-checkout` | ❌ revised source manifest created by Recovery 3B | ⬜ pending |
...
| T-167-18 | CI hermeticity is obtained by weakening proof or dynamically widening repairs | Require shallow RED/GREEN, a closed authorized repair universe intersected with plan files, and unchanged recurring leaf/failure authority. |
...
| T-167-27 | Post-merge evidence or current authority becomes self-referential, local-only, or silently unlanded | Assign the exact receipt/summary/verification/final ROADMAP/final STATE five-path set to Phase 168's first reversible landing. |
