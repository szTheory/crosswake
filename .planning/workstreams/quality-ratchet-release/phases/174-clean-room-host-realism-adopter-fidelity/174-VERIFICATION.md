...
| 4 | **ROOM-04** — legacy path's `step=` logging reaches grep-able parity with the matrix path | ✓ VERIFIED, and ROADMAP SC#4's more specific CI-log-to-CI-log method is now **FULLY MEASURED AND GUARDED** | `LEGACY_STEP_MARKERS`/`legacy_step_marker()` present in `script/verify_companion_cleanroom.sh`; roster-in-log completeness asserted by the phase174_cleanroom_host_realism_test.exs suite. Both sides of SC#4 now exist as real captured CI logs: matrix `evidence/174-matrix-ci-run.log` (run 35324177344, 6 distinct markers) and legacy `evidence/174-rindle-ci-run.log` (run 35366337182, 9 distinct markers). Finding A — which disclosed that the measurement itself was asserted by nothing — was closed 2026-09-18 by adding four tests to `phase174_cleanroom_lane_parity_test.exs`: a non-emptiness gate on both marker sets that runs BEFORE any comparison, the legacy ≥ matrix comparison, set equality against the `LEGACY_STEP_MARKERS` roster read from the harness script, and a D-14 truncation control. Demonstrated red by three mutations (see the mutation-evidence table below). |
...
| `test/crosswake/proof/phase174_cleanroom_lane_parity_test.exs` | Roster parity + SC#4 marker parity, 10 tests | ✓ VERIFIED on both halves. Finding A closed 2026-09-18: the four added tests read both captured CI logs, gate on a non-empty `step=` marker set on each side *before* comparing, assert legacy ≥ matrix, and assert the legacy log's marker set equals the `LEGACY_STEP_MARKERS` roster read from `script/verify_companion_cleanroom.sh`. | Present on disk |
...
| `.github/workflows/clean-room-proof-rehearsal.yml` | GitHub `workflow_dispatch` API | `gh workflow run` | ✓ **WIRED** (2026-09-18) | The default-branch blocker cleared when PR #184 merged as `bb570820`. Dispatched against `--ref main`; run `35366337182`, job `Clean-room proof rehearsal`, conclusion `success`, against the live published `crosswake_rindle 0.1.0`. Log captured at `evidence/174-rindle-ci-run.log`. ROOM-03 satisfied. |
| `test/crosswake/proof/phase174_cleanroom_lane_parity_test.exs` | `evidence/174-matrix-ci-run.log` / `evidence/174-rindle-ci-run.log` | assertion reading the logs | ✓ **WIRED** (2026-09-18, Finding A closure) | Four tests read both logs, gate on a non-empty `step=` marker set on each side before comparing, assert legacy ≥ matrix, and assert the legacy log's marker set equals the harness's declared `LEGACY_STEP_MARKERS` roster. Read paths resolve live-or-archived, so the guard survives milestone archival. |
...
| `phase174_cleanroom_lane_parity_test.exs` — SC#4 marker half (174-04; guard added at Finding A's closure, 2026-09-18) | **Three mutations, all demonstrated red.** Matrix log truncated to zero bytes → 10 tests / 3 failures. Legacy CI log truncated to zero bytes → 10 tests / 3 failures. `step=doctor` renamed to `step=physician` in the legacy CI log → 10 tests / 1 failure, reporting `Declared but never emitted: ["doctor"]; emitted but not declared: ["physician"]`. Unmutated tree → 10 tests / 0 failures; every evidence file restored byte-identical (`git diff --stat` on `evidence/` empty). The original escape — moving or truncating the matrix log left the suite at 6 tests / 0 failures — is what this guard closes. | **closed** |
