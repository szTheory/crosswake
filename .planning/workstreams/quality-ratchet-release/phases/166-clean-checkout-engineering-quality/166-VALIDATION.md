...
| 166-W0-03 | TBD | 0 | ENG-03 | NUL-safe status parsing, exact cleanup prefixes, and non-staging drift checks preserve repository state. | unit + integration | `node --test test/js/repository_verification.test.mjs` | ❌ W0 | ⬜ pending |
| 166-W0-04 | TBD | 0 | ENG-04 | Diagnostics expose no contents or environment secrets and emit one bounded remediation command. | golden + negative output | `script/verify_repository.sh --self-test` | ❌ W0 | ⬜ pending |
