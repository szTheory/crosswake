...
| Prioritize runtime contract confidence before new feature breadth | Post-v13 repo inspection found bridge protocol truth split between `1.1.0` Elixir contract posture and `1.0.0` manifest/example/native proof paths; adopters need one canonical runtime contract before more capability or provider breadth | Active in v14.0 (started 2026-06-20) |
| Reconcile bridge-protocol compatibility to a single `>=` min-version-floor semantics across Elixir and native (change native exact-equality to negotiate by floor) | Elixir already negotiates by `>=` while native demands exact equality — the mismatch is what silently denies valid additive-bump requests; every mature wire protocol (Postgres, TLS 1.3, Phoenix Channels `vsn`) abandoned exact-match | — Pending (v14.0 COMPAT-01) |
