...
**Phase 43 (Rulestead Hermetic+Advisory Proof And Guide) complete — `2026-05-31`.** The rulestead seam now has a dedicated `phase43-proof.yml` with a merge-blocking hermetic job that runs without the optional dependency and an advisory job that includes `rulestead` behind `MIX_INCLUDE_RULESTEAD=1`. GitHub branch protection on `main` requires the strict `merge-blocking rulestead proof (hermetic)` status check; PR #9 proved the gate. `guides/companions.md` now contains the companion-pattern intro plus the rulestead slice, locked by `test/crosswake/guides/companions_test.exs`. Phase 43 security is verified with `threats_open: 0`; the optional-dep supply-chain risk is accepted only for advisory CI. Full hermetic suite: 391 tests, 0 failures; docs-contract: 9 tests, 0 failures.
...
---
*Last updated: 2026-05-31 — Phase 43 complete: rulestead hermetic+advisory proof split shipped, `main` now requires the live `merge-blocking rulestead proof (hermetic)` GitHub status check, the rulestead guide slice is docs-contract locked, and security verification has `threats_open: 0`. PROOF-01 satisfied for the rulestead companion; the full PROOF-02 companion guide remains Phase 47 scope.*
