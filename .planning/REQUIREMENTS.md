...
- [ ] **PROP-02**: The Plug emits `[:crosswake, :threadline, :request, :start|:stop|:exception]` telemetry carrying only low-cardinality metadata (`thread_id`, `correlation_id`, `route_id`, `source`), and rejects forbidden/PII keys via the shared allowlist guard.
...
- [ ] **PROP-04**: `thread_id` is a first-class field layered above the unchanged per-command `correlation_id` on the bridge and activation contracts, carried across `cold_start → deep_link → notification → in_app_navigation` activations.
