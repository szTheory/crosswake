...
- [ ] **DECOUPLE-04**: An absent or misconfigured auth companion on an auth-predicated route (`auth_min_level`/`requires_recent_auth`/`auth_posture` set) fails closed — denies with `:dependency_missing`; a companion raising during evaluation is rescued and treated as deny. "No evaluation = allow" is reachable only on non-auth routes; multiple `auth_authority?/0` companions resolve to the first registered plus a doctor warning. (D-3)
- [ ] **DECOUPLE-05**: Core ships a hardcoded baseline PII forbidden-metadata-key denylist (auth tokens + identity fields) that the safe logger **always** applies regardless of which companions are present, layered above the per-companion runtime aggregation — so an absent/misconfigured companion can never silently drop token/identity scrubbing. (D-5)
