...
- [x] **SYNC-01**: Playwright E2E tests for the offline study island use actual network toggling (`page.context().setOffline(true)`) rather than faking `localStorage` behavior.
...
- [x] **OFFC-01**: A `mix crosswake.gen.offline_ui` generator scaffolds a host-owned `OfflineController` and `.html.heex` template to serve the vanilla-JS offline island.
- [x] **OFFC-02**: The generated Offline UI explicitly disconnects from the LiveView websocket lifecycle, preventing runtime errors when entirely offline.
...
- [x] **BRND-01**: The generated offline UI applies the Crosswake Brand Book design tokens natively via Tailwind (e.g., Wake Green, Brass) without bundling heavy JS frameworks like React or Alpine.
- [x] **BRND-02**: The offline UI implements explicit, honest microcopy (e.g., "Available offline", "Complete server confirmation", "Draft only", "Cached read-only") for all states rather than relying on ambiguous color signals.
...
| OFFC-01 | Phase 101 | Complete |
| OFFC-02 | Phase 101 | Complete |
| BDGT-01 | Phase 100 | Complete |
| BDGT-02 | Phase 100 | Complete |
| BDGT-03 | Phase 100 | Complete |
| BRND-01 | Phase 101 | Complete |
| BRND-02 | Phase 101 | Complete |
